...
while true; do
    ...
    # Check if the file has a .chk extension
...
for script in 03_correct_mol2.py 04_mol2_parmch2.sh 05_prepare_correct_mol2.py; do
    if [[ $script == *.py ]]; then
...
for script in 06_get_atom_type.py 07_preclean_forcefield.py 08_prepare_correct_forcefield.py 09_prepare_correct_forcefield_pos.py 10_postclean_forcefield.py 11_include_uffdata.py; do
    if [ -f "$SCRIPT_DIR/$script" ]; then
        python3 "$SCRIPT_DIR/$script" "$RUN_DIR"
        if [ $? -ne 0 ]; then
            echo "Failed to execute $script. Exiting."
...
mv updated_updated_COMPLEX_modified2.frcmod COMPLEX.frcmod
...
python3 "$SCRIPT_DIR/12_update_lib.py"

# Remove the unnecessary file
rm dihedral.dat distance.dat esout atom_type.dat COMPLEX_modified.mol2 COMPLEX_modified.frcmod complex.fchk forcefield2.dat forcefield.dat metal_number.dat new_atomtype.dat temp_COMPLEX_modified.frcmod temp.dat updated_COMPLEX_modified.frcmod updated_COMPLEX_modified2.frcmod angle.dat bond_angle_dihedral_data.dat qout punch QOUT ANTECHAMBER* ATOMTYPE.INF leap.log
...
rm similar.dat input_library.tleap distance_type.dat 
